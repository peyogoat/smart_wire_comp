...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueSystime = gizProtocolExchangeBytes(gizY2X(SYSTIME_RATIO,  SYSTIME_ADDITION, dataPoints->valueSystime)); 
    devStatusPtr->valueConsumption = gizProtocolExchangeBytes(gizY2X(CONSUMPTION_RATIO,  CONSUMPTION_ADDITION, dataPoints->valueConsumption)); 
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSystime != cur->valueSystime)
    {
        ...
        {
            os_printf("Systime Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueConsumption != cur->valueConsumption)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
